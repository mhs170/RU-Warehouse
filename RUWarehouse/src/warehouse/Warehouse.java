...
public class Warehouse {
    ...
    private void addToEnd(int id, String name, int stock, int day, int demand) {
        ...
        Product obj = new Product(id, name, stock, day, demand);
        int sectorNum = id % 10;
        sectors[sectorNum].add(obj);
    }
    ...
    public void restockProduct(int id, int amount) {
        ...
        for (int i = 1; i < size + 1; i++){
            int newID = sectors[sectorNum].get(i).getId();
            if (newID == id){
    ...
    public void deleteProduct(int id) {
        ...
        int sectorNum = id % 10;
        int size = sectors[sectorNum].getSize();
        for (int i = 1; i < sectors[sectorNum].getSize() + 1; i++){
            int newID = sectors[sectorNum].get(i).getId();
            if (newID == id){
                sectors[sectorNum].swap(size, i);
                sectors[sectorNum].deleteLast(); 
    ...
    public void purchaseProduct(int id, int day, int amount) {
        ...
        for (int i = 1; i < size + 1; i++){
            int newID = sectors[sectorNum].get(i).getId();
            if (newID == id && amount < sectors[sectorNum].get(i).getStock()){
